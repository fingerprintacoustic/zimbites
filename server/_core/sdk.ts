...
class SDKServer {
  ...
  async signSession(
    ...
    // Ensure appId is never empty to avoid verification failure
    if (!jwtPayload.appId) {
      jwtPayload.appId = "zimbites-default-id";
    }
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    console.log("[Auth] Found Session Cookie:", !!sessionCookie);
    
    const session = await this.verifySession(sessionCookie);

    if (!session) {
      console.warn("[Auth] No valid session found for cookie");
    ...
    // If user not in DB, sync from session info first (faster for dev login)
    if (!user) {
      try {
        await db.upsertUser({
    ...
    // Fallback: sync from OAuth server if still no user
    if (!user) {
      try {
        const userInfo = await this.getUserInfoWithJwt(sessionCookie ?? "");
        await db.upsertUser({
