...
async function createTestOrder() {
  ...
  const [result] = await connection.execute(
    `INSERT INTO orders (orderNumber, customerId, restaurantId, deliveryAddress, status, paymentMethod, subtotal, deliveryFee, platformCommission, total, createdAt, updatedAt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
    [orderNumber, 90001, 360002, "123 Harare Street", "pending", "cash", 4500, 500, 350, 5350]
  );
